...
int __stdcall MyStartPage(HDC hDC)
{
	...
	//Append mode
	pTextOutFile = fopen("E:\\VCPlusPlus\\ShellAboutDetour01\\RunMe\\TextOutFile.txt", "a+");
...
//	hdc = global_hDC01;
	fwprintf(pFile, L"Restoring first hdc = %d\n", hdc);
...
int __stdcall MyPrintDlgW(LPPRINTDLGW lpprintdlgw01)
{
	...
	fwprintf(pFile, L"hPrintTemplate = %d\n", lpprintdlgw01->hPrintTemplate);
	fwprintf(pFile, L"hSetupTemplate = %d\n", lpprintdlgw01->hSetupTemplate);
	fwprintf(pFile, L"hwndOwner = %d\n", lpprintdlgw01->hwndOwner);
	fwprintf(pFile, L"lCustData = %d\n", lpprintdlgw01->lCustData);
	fwprintf(pFile, L"lpfnPrintHook = %d\n", lpprintdlgw01->lpfnPrintHook);
	fwprintf(pFile, L"lpfnSetupHook = %d\n", lpprintdlgw01->lpfnSetupHook);
	fwprintf(pFile, L"lpPrintTemplateName = %d\n", lpprintdlgw01->lpPrintTemplateName);
	fwprintf(pFile, L"lpSetupTemplateName = %d\n", lpprintdlgw01->lpSetupTemplateName);
...
__declspec(dllexport) INT APIENTRY DllMain(HMODULE hDLL, DWORD Reason, LPVOID Reserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		pFile = fopen("E:\\VCPlusPlus\\ShellAboutDetour01\\RunMe\\output01.txt", "w+");

        char dateStr [9];
        char timeStr [9];
        _strdate( dateStr);
        fprintf( pFile, "The current date is %s \n", dateStr);
        _strtime( timeStr );
        fprintf( pFile, "The current time is %s \n", timeStr);
		...
		DetourAttach(&(PVOID&)pStartDocPrinterW, MyStartDocPrinterW);
	...
	case DLL_PROCESS_DETACH:
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
		DetourDetach(&(PVOID&)pAddJobW, MyAddJobW);
		DetourDetach(&(PVOID&)pStartDocPrinterW, MyStartDocPrinterW);
		DetourDetach(&(PVOID&)pPrintDlgW, MyPrintDlgW);
		DetourDetach(&(PVOID&)pPrintDlgExW, MyPrintDlgExW);
		...
		DetourDetach(&(PVOID&)pPolyTextOutA, MyPolyTextOutA);
		DetourDetach(&(PVOID&)pPolyTextOutW, MyPolyTextOutW);
